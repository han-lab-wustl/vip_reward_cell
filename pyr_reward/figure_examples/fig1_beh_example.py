...
for ii in iis:
   ...
   # reindex trials
   epblocklen = [np.arange(0,eps[2]-ep1),np.arange(eps[2]+1-ep1,ep2)]
   trials_all = trials.copy()
   trials_all[epblocklen[1][0]-1:] =trials_all[epblocklen[1][0]-1:]+trials[:epblocklen[0][-1]].max() + 1
   realtrialn=np.unique(trials)
   incorrlick=np.zeros_like(lick[mask])
   for tn,trial in enumerate(np.unique(trials_all)):
      tr = trials_all==trial
      realtrial = np.unique(trials[trials_all==trial])[0]
      # print(trial,realtrial)
      if realtrial>2:
