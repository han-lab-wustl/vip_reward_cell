...
savepth = os.path.join(savedst, 'nonrew_assemblies.pdf')
    ...
    # if df.iloc[ii].animals==an_plt and df.iloc[ii].days==an_day:
        ...
        for jj,asm in enumerate(ass_all):
            ...
            if plot:
                ...
                fig.suptitle(f'Non-reward ensemble \n {df.iloc[ii].animals}, {df.iloc[ii].days} \n\
                ...
                if jj==2:
                    plt.savefig(os.path.join(savedst,f'{an_plt}_{an_day}_nonrew_ensemble_eg.svg'),bbox_inches='tight')
...
ax.set_title('Non-reward ensembles', pad=50)
plt.tight_layout()
plt.show()
df_clean.to_csv(r'Z:\condition_df\nonrew_ensemble.csv', index=None)