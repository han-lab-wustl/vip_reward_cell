...
title = ['Pre-reward, Single cell', 'Pre-reward, Population','Lick','Velocity','Near reward Place, Single cell', 'Place, Population']
# get near reward place cells
pc_coms_rewrel = coms_rewrel[:,pcs]
pc_coms_rewrel = np.nanmedian(pc_coms_rewrel,axis=0)
nearewpc = np.where(((pc_coms_rewrel<0) & (pc_coms_rewrel>-np.pi/4)))[0]
nearewpceg = nearewpc[1]
fig,axes=plt.subplots(nrows=6,ncols=4,sharex=True,sharey='row',figsize=(8,9))
tcs = [tcs_correct_abs[:,pre_goal_cells[1],:],tcs_correct_abs[:,pre_goal_cells,:],np.squeeze(lick_correct_abs),np.squeeze(vel_correct_abs),tcs_correct_abs[:,nearewpceg,:],tcs_correct_abs[:,nearewpc,:],]
for ep in range(len(tcs_correct_abs)):
   for jj,tc in enumerate(tcs):
      ...
      if ep==0: 
         ...
         ax.set_title(title[jj])
         
      ax.spines[['top','right']].set_visible(False)
fig.suptitle(f'mouse {animal[1:]}, session {day}\nPre-reward cells vs. near reward place cells')
# plt.tight_layout()
...
colors=['k','slategray','darkcyan','darkgoldenrod']
cell_color=['cornflowerblue','indigo']
lbls = ['$\Delta F/F$','(Hz)', '(cm/s)','$\Delta F/F$',]
title = ['Pre-reward, Population activity','Lick','Velocity','Near reward Place, Population activity']
# get near reward place cells
pc_coms_rewrel = coms_rewrel[:,pcs]
pc_coms_rewrel = np.nanmedian(pc_coms_rewrel,axis=0)
nearewpc = np.where(((pc_coms_rewrel<0) & (pc_coms_rewrel>-np.pi/4)))[0]
nearewpceg = nearewpc[1]
fig,axes=plt.subplots(nrows=4,ncols=4,sharex=True,sharey='row',figsize=(8,9))
tcs = [tcs_correct_abs[:,pre_goal_cells,:],np.squeeze(lick_correct_abs),np.squeeze(vel_correct_abs),tcs_correct_abs[:,nearewpc,:],]
for ep in range(len(tcs_correct_abs)):
   for jj,tc in enumerate(tcs):
      ax=axes[jj,ep]
      if jj==0: color=cell_color[0]
      elif jj==3: color=cell_color[1]
      else: color=colors[ep]
      # population
      if jj==0 or jj==3: 
      ...
      if ep==0: 
         ax.set_ylabel(lbls[jj])
         if jj==3: ax.set_xlabel('Track position (cm)')
         ax.set_title(title[jj])
         
      ax.spines[['top','right']].set_visible(False)
fig.suptitle(f'Pre-reward cells vs. near reward place cells')
# plt.tight_layout()
plt.savefig(os.path.join(savedst,'place_lick_vel_eg.svg'), bbox_inches='tight')
#%%
# fig3 a
...
plt.rc('font', size=12)
fig, axes = plt.subplots(ncols=4,nrows=3, figsize=(7,6), sharex=True, sharey='row',height_ratios=[3,1,1])
...
for en,ep in enumerate([0,1,2,3]):
   ...
   pltt = tcs_correct_abs[ep, all_goal_cell, :]
   pltt = np.array(pltt)
   # Sort by COM
   pltt = pltt[np.argsort(coms_correct_abs[0][all_goal_cell])]
   # Normalize each row
   ...
   im_list.append(im)   
   ax.axvline(rewlocs[ep]/bin_size, color='w', linestyle='--',linewidth=3)
   if ep == 0:
      ax.set_ylabel('Reward cell # (sorted)')
   ax.set_title(f'Epoch {en+1}\n Reward at {int(rewlocs[ep])} cm',fontsize=14)
   ax=axes[1,en]
   ax.plot(lick_correct_abs[ep][0],color=colors[en])
   ax.spines[['top', 'right']].set_visible(False)
   ax.axvline(rewlocs[ep]/bin_size, color='k', linestyle='--',linewidth=3)
   if ep == 0:ax.set_ylabel('Lick rate (Hz)')
   ax=axes[2,en]
   ax.plot(vel_correct_abs[ep][0],color=colors[en])
   ax.spines[['top', 'right']].set_visible(False)
   ax.axvline(rewlocs[ep]/bin_size, color='k', linestyle='--',linewidth=3)
   if ep == 0:
...
fig.suptitle(f'mouse {animal[1:]}, session {day-50}\nAll reward cells')
plt.tight_layout()
plt.savefig(os.path.join(savedst, f'{animal}_{day}_fig3_rew_map.svg'), bbox_inches='tight')
#%%