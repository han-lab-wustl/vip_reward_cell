...
for ii in iis:
   ...
   if (animal!='e217') & (conddf.optoep.values[ii]<2):
      ...
      pcs = np.array([xx for xx in pcs if xx not in goal_cells and xx not in post_goal_cells])
      com_pc = np.nanmedian(coms_rewrel[:,pcs],axis=0)
      # only near pcs?
      pcs=pcs[(com_pc<0) & (com_pc>-np.pi/4)]
      ...
      if len(dff_lick_corr)>0:# all correct
         ...
         plt.figure()
         sns.barplot(x='condition',y='mean_dff_near',data=df)
         ######### SAVE
         df_all.append(df)
...
plt.tight_layout()
plt.savefig(os.path.join(savedst, 'current_v_old_place_summary_meandff.svg'), bbox_inches='tight')
pltdf.to_csv(r'C:\Users\Han\Documents\MATLAB\vip_reward_cell\raw_data\fig3h.csv')
...
fig.suptitle(r'Place (COM $<\pi/4$ and $>0$)')
plt.tight_layout()
plt.savefig(os.path.join(savedst, 'current_v_old_place_meandff.svg'), bbox_inches='tight')
bigdf.to_csv(r'C:\Users\Han\Documents\MATLAB\vip_reward_cell\fig3\fig3i.csv')
