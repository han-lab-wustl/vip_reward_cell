...
for ii in range(len(conddf)):
    ...
    if ('vip' not in in_type) & (conddf.optoep.values[ii]<2):        
        ...
        goal_per_perm = [len(xx)/len(coms_correct[0]) for xx in com_goal]
        ...
        for ii,xx in enumerate(rates_per_perm):
            print(f'rate difference: {xx}, rew cell prop: {goal_per_perm[ii]}')
...
fig.suptitle('Reward cell % vs. performance metrics')
plt.tight_layout()
plt.savefig(os.path.join(savedst, "performance_v_rewardcell.svg"))
#%%
df=df.groupby(['animals']).mean(numeric_only=True).reset_index()
fig,axes = plt.subplots(ncols=3,figsize=(11,4),sharey=True)
metrics = ['rates', 'lick_selectivity', 'lick_rate']
...
fig.suptitle('Reward cell % vs. performance metrics, per mouse')
plt.tight_layout()
plt.savefig(os.path.join(savedst, "performance_v_rewardcell_permouse.svg"))